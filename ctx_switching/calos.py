...
class CalOS:
    ...
    def timer_isr(self):
        ...
        '''
        CalOS.current_proc.set_registers(self._cpu.get_registers()) # Save the CPU's registers into the current PCB
        if (self._ready_q.count != 0):
            self.context_switch() # Perform a context switch
    ...
    def context_switch(self):
        ...
        '''
        CalOS.next_proc = self._ready_q.pop(0) # Set the next PCB as the next program on the queue
        CalOS.current_proc.set_registers(self._cpu.get_registers()) # Store the CPU's registers in the current PCB
        self._cpu.set_registers(CalOS.next_proc.get_registers()) # Set the CPU's registers to the next PCB
        self.add_to_ready_q(CalOS.current_proc) # Add the original PCB to the ready queue
        CalOS.next_proc.set_state("RUNNING") # Set the new PCB's state to running
        CalOS.current_proc = CalOS.next_proc # Set the current PCB as the running PCB

    def run(self):
        '''Startup the timer controller and execute processes in the ready
        queue on the given cpu -- i.e., run the operating system!
        '''
        if (self._ready_q.count != 0): # If the queue isn't empty
            CalOS.current_proc = self._ready_q.pop(0) # Set the current PCB to the first program on the ready queue
            self.reset_timer() # Reset the timer
            self._cpu.set_registers(CalOS.current_proc.get_registers()) # Set the CPU's registers as the current process' registers
            self._cpu.run_process() # Run the process
            CalOS.current_proc.set_state("DONE")
