...
class Monitor:
        ...
        while True:
            try:
                ...
                if instr == '?':
                    ...
                    print("R <addr>: run the programs whos starting locations are in the array at addr")
                    continue
    ...
    def _run_batch(self, addr):
        # creates a new thread, passing in ram, the os, the
        # starting address, and batch to true
        self._cpu = CPU(self._ram, calos.CalOS(), addr, self._debug, batch=True)
        self._cpu.start()		# call run()
        self._cpu.join()		# wait for it to end
