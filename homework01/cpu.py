...
class CPU(threading.Thread):
    ...
    def set_programs(self, addr): # setter
        self._prog_list = self._ram[addr]
    ...
    def run(self):
        ...
        if self._batch == False:
            self._run_program()
        else: # you're in batch mode
            while (self._ram[self._array_address] != 0): # check what program you want to run
                self.set_programs(self._array_address)  # get the value in the memory of the array address
                self._registers['pc'] = self._prog_list # set the pc
                self._registers['reg0'] = 0 # clear the registers
                self._registers['reg1'] = 0
                self._registers['reg2'] = 0
                
                self._run_program() 
                print("Program Complete")
                self._array_address += 1 # increment the address
